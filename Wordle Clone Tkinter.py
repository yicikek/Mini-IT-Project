...
def gamegui():
    ...
    gametitle.place(x=200, y=30)
        
    tries = 0
    selected_word = str(select_word())

    # Input for word guess
    word_input = tk.Entry(game)
    word_input.grid(row=999, column=0, columnspan=3, padx=10, pady=10)
    
    # Button to submit guess
    word_guess_button = tk.Button(game, text="Submit", command=lambda:[get_guess(), clear_text(word_input)])
...
def rules():
    ...
    line3 = tk.Label(frame3, text='2. A correct letter placed in the correct order will turn into BOLD', font=('Arial', 13))
    line3.place(x=100, y=190)
    line4 = tk.Label(frame3, text='3. A correct letter placed in the wrong order will turn into ITALIC', font=('Arial', 13))
    line4.place(x=100, y=230)
    line5 = tk.Label(frame3, text='4. An incorrect letter will remain with the default text styles', font=('Arial', 13))
    ...
    b4 = tk.Button(frame3, text="I understand", cursor='hand2', font=('Arial', 15), command=gamegui)
